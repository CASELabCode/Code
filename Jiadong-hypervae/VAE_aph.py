...
def loss_function_v2(recon_x, x, mu, log_var):
    smoothness_loss =  torch.mean((recon_x[:, 1:]- recon_x[:, :-1]) ** 2)
    smoothed_recon_x = smooth_output_gaussian(recon_x)
    L1 = F.l1_loss(smoothed_recon_x, x, reduction='mean')
    total_loss = L1 +0.1*smoothness_loss
...
# def loss_function(recon_x, x, mu, log_var, delta=0.04):
#     # Huber Loss (Smooth L1 Loss)
#     L1 = F.smooth_l1_loss(recon_x, x, beta=delta, reduction='mean')   
#     loss = L1 
#     return loss
...
def evaluate(model, test_dl):
    ...
    with torch.no_grad():
        for x, y in test_dl:
            ...
            #y_pred_s = smooth_output_gaussian(y_pred)
...
#     train_real_dl = DataLoader(train_dataset_real, batch_size=train_size, shuffle=True, num_workers=0, pin_memory=True)
#     test_real_dl = DataLoader(test_dataset_real, batch_size=test_size, shuffle=False, num_workers=0, pin_memory=True)
...
if __name__ == '__main__':
    ...
    save_dir = "F:\\aphy-chla-predictions\\plots\\VAE_aph_EMIT"
